...
In 1911, Alwin Korselt showed that Ernst Schröder's proof of the Cantor-Bernstein-Schröder theorem, originally published in 1898 @intro-schroder1898, contained an error @intro-korselt1911. While the theorem was correct, and other proofs of this theorem existed even back then, this isn't the only instance where mathematicians tried to proof a statement, thought they conceived such proof and later found that the proof was incorrect or incomplete. Thus, there exist software tools called proof assistants which allow one to formalize mathematical proofs and have the computer check for the proof's correctness. One such tool is _Isabelle_ #footnote[https://isabelle.in.tum.de/], originally developed at the _University of Cambridge_ and _Technical University of Munich_.
...
// , such that building Isabelle support for a new code editor becomes possible.
...
As a temporary workaround, we used the Isabelle fork `isabelle-emacs` #footnote[https://github.com/m-fleury/isabelle-emacs], which includes many advancements to the Isabelle language server in order to support the _Emacs_ #footnote[https://www.gnu.org/software/emacs/] code editor. Based on this fork, a usable Neovim Isabelle environment was achievable, but there were still many missing features compared to #jedit. The `isabelle-emacs` fork was thus a strong inspiration for some of the changes done in the context of this thesis.
