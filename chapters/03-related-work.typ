...
#cite(<lsp-spec-extension>, form: "prose") explored extensions to the LSP specification to support more types of semantic languages, including theorem provers. With these extensions implemented in both the specification as well as language clients, it may be possible to update the Isabelle langauge server to use these new LSP extensions and support other language clients with almost no extra work (i.e. no custom handlers for custom LSP messages).
