...
#cite(<lsp-spec-extension>, form: "prose", style: "../association-for-computing-machinery-modified.csl") explored extensions to the LSP specification to support more types of semantic languages, including theorem provers. With these extensions implemented in both the specification and language clients, it may be possible to update the Isabelle language server to use these new LSP extensions and support other language clients with almost no additional work (i.e. no custom handlers for custom LSP messages).
