...
Our original goal included making the Isabelle language server more flexible, rendering language clients for other code editors feasible. In order to assess the server's new flexibility with our advancements, we built two prototype language clients for the Neovim and Sublime Text code editors.

== Neovim

Originally the motivation behind this thesis, our Neovim language plugin has seen great usability improvements. We found the greatest boon in user experience to be the highlighting in output and state panels. Especially state content has become significantly easier to read thanks to proper highlighting.
...
Utilizing the Sublime LSP package #footnote[https://lsp.sublimetext.io/], we found that a working Sublime Text Isabelle language package was doable in only $~200$ lines of python code/* , the result of which can be seen in @fig:sublime-isabelle */. This package gave support for dynamic highlighting, a working output panel with highlighting, active markup via code actions, completions for symbols, and conversion of symbols from ASCII representations to Unicode. All that on top of more typical LSP features like diagnostics. Although still rough around the edges, we found this package to already be usable for basic theories.
...
/ Minor Advancements: Many further small improvements could be made to the language server. These are things that are fairly easy and obvious to build, but just require some additional work.
