...
Output panel content is also created as such XML bodies. An Isabelle theory consists of many _Commands_, each _Command_ has some _Command Result_, and when the caret is above a command, the content of its _Command Result_ is displayed on the Output panel. And these _Command Results_ are also just XML bodies. Similarly, State panel content is also originally just an XML body.
