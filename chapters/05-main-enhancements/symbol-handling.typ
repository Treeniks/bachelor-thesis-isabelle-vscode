...
#vscode uses such a hard-coded list of symbols. This list is added into the custom #utf8isa encoding while building the patched VSCodium. It also only includes the default set of symbols Isabelle offers out of the box, it does not include custom user additions. As this list is hard-coded, any change in the list of symbols would also require recompiling #vscode. This is different to #jedit, where the code of the #utf8isa encoding exists within #scala and therefore has access to the complete list of symbols.
