...
Additionally, we added the ability to disable HTML output for state and output panels, a feature primarily motivated by the requirements of the Neovim language client prototype mentioned in @intro:motivation. We also provided more granular control for the language client with regards to the handling of Isabelle symbol, which enhances the server's flexibility.
