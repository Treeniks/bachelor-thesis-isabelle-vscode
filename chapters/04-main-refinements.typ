...
_Output_ panel content is also created as such XML bodies. An Isabelle theory consists of many _Commands_, each with some _Command Result_ which is an XML body. When the caret is above a _Command_, the content of its _Command Result_ is displayed on the _Output_ panel. _State_ panel content is similarly just an XML body.
...
#footnote[One particular difference between #vscode['s] and #jedit['s] implementation of the _UTF-8-Isabelle_ encoding is that the set of Isabelle Symbols that #vscode[] understands is static. It is possible to extend this set and #jedit[] can deal with newly defined symbols while #vscode[] can not, although this is rarely a problem in practice.]
However, the language server must still obtain the contents of the file.

// #info[One particular difference between #vscode['s] and #jedit['s] implementation of the _UTF-8-Isabelle_ encoding is that the set of Isabelle Symbols that #vscode[] understands is static. It is possible to extend this set and #jedit[] can deal with newly defined symbols while #vscode[] can not, although this is rarely a problem in practice.]
