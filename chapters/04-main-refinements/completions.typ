...
The LSP specification defines how completions are supposed to be handled. For this, it defines a `textDocument/completion` request sent by the client @lsp-spec. In particular, this means that the choice of _when_ completions are triggered is up to the client. The client may send completion requests whenever it wants. They typically trigger them automatically for certain trigger characters that were typed, although most of them also offer a keybind to manually request completions. The client may choose which characters specifically trigger these automatic requests, and many also offer the user to overwrite this set in their preferences. The language server can additionally send a list of trigger characters within its capabilities during the initialization stage described in @back:lsp-initialization.

Completions were already implemented in the Isabelle language server. The generation of the completions used the same internal system that #jedit uses, meaning the completions sent by the language server should have been identical as well.
...
Curiously, in #vscode, only categories 2 and 3 worked, category 1 did not. In Neovim, categories 1 and 3 worked, and 2 did not. Since completions are part of the standard LSP spec, handling of these completions is not done within the Isabelle extensions, but instead the standard language client implementations. Although in Neovim, the standard language client does not offer completions out of the box, so an additional completion plugin is required, for which we used `nvim-cmp` #footnote[https://github.com/hrsh7th/nvim-cmp].

The core problem is that completions in the LSP are meant to work in an additive fashion. That is, if the user write `con` in JavaScript, it should be able to complete to `console`. Note that the text the user wrote originally is a prefix of the completed text. This is not necessarily the case for Isabelle: In #jedit, the user can complete #isabelle(`\Longright`) to #isabelle(`\<Longrightarrow>`) which in turn gets displayed as #isabelle(`⟹`). An abbreviation like #isabelle(`<=`) should get replaced by #isabelle(`\<le>`) which gets displayed as #isabelle(`≤`). For the language server, it may even be that the completion should insert #isabelle(`⟹`) or #isabelle(`≤`) directly, depending on the options described in @symbol-options. There is many such _flex_ completions, which the LSP is ill-equipped to handle.

However, the Isabelle language server does send these completions anyway, it's up to the client whether it wants to display them or not. Clients are supposed to implement a filter for these completions, and this filter is intentionally not defined in the LSP specification in order to allow consistency across languages within the editor. Therefore, different clients may use vastly different approaches for these filters, causing our inconsistency issues.

There are discussions for supporting flex completions within the LSP #footnote[https://github.com/microsoft/language-server-protocol/issues/651]. For our use, we made use of the `filterText` string that is optionally provided with completions. This text is supposed to be used when filtering. By setting this text to be equivalent to what the user wrote up to the completion request (#isabelle(`\Longright`) in our previous example), this should force all filters to show the completion no matter what.
...
Note that this feature seems not supported by all language clients. Both Neovim and Sublime Text seemed to ignore this list in our testing, although VSCode does.
